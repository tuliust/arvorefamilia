  ...
  return (
    <div className="min-w-0 space-y-3 border-t border-gray-100 pt-4">
      ...
      {comentarios.length === 0 ? (
        <p className="break-words rounded-md bg-gray-50 p-3 text-sm text-gray-500">Nenhum comentário ainda.</p>
      ) : (
            ...
            <div key={comentario.id} className="min-w-0 rounded-md bg-gray-50 p-3">
              <div className="flex min-w-0 items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="mt-1 whitespace-pre-wrap break-words text-sm leading-6 text-gray-700">{comentario.conteudo}</p>
                </div>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-2 sm:flex-row">
        <Textarea
          ...
          className="min-h-16"
        />
        <Button type="submit" disabled={enviando || !conteudo.trim()} className="w-full sm:w-auto sm:self-start">
          Comentar
        </Button>
      </form>
    </div>
