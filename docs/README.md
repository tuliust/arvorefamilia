...
> Status: índice canônico atualizado após remoção das views antigas da árvore, simplificação do painel e higiene do repositório.

Este diretório concentra a documentação técnica, funcional, operacional e histórica do projeto **Árvore Família**.

Use este arquivo como ponto de entrada antes de consultar guias específicos. A documentação canônica deve registrar apenas comportamento implementado, decisões vigentes ou pendências explicitamente classificadas como backlog/QA.
...
- o painel de controles foi simplificado e não possui mais a barra visual `Filtros | Legendas | Ações`;
- o painel desktop/mobile preserva:
  - Zoom +;
  - Zoom -;
  ...
  - Exportar;
  - Destacar;
  - filtros diretos/status visíveis sem abas internas;
...
| `INVENTARIO_TECNICO.md` | Rotas, componentes, services, tipos, CSS, testes e documentação. | Atualizado. |
...
| `GUIA_CORRECAO_ERROS.md` | Troubleshooting por sintoma, causa provável e correção. | Atualizado. |
...
| `ARVORE_LEGENDAS_CONECTORES_PAINEL.md` | Painel, filtros, controles, destaques, conectores e exportação. | Atualizado após simplificação. |
...
Documentos sobre antigas views de árvore devem ser arquivados ou marcados como legado:
...
- `genealogia` pode aparecer como conceito visual da horizontal;
...
A frente de refatoração da árvore fechou com:
...
- resolver legado removido;
- backups/env locais removidos do versionamento e ignorados.
