...
export function LifeStatusKpiGrid({
  ...
  return (
    <section className="min-w-0">
      <div className="grid w-full min-w-0 grid-cols-[repeat(3,minmax(0,1fr))] gap-[clamp(0.4rem,1.15vh,0.7rem)]">
        {items.map((item) => {
          ...
          return (
            ...
            >
              <span className="block truncate text-[clamp(11px,1.65vh,13px)] font-semibold leading-tight">{item.label}</span>
              <span className="mt-[clamp(0.25rem,0.65vh,0.4rem)] block truncate text-[clamp(18px,2.55vh,22px)] font-bold leading-none">{item.value}</span>
            </button>
          );
        })}
      </div>
