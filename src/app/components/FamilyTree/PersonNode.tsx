...
export const PersonNode = React.memo(({ data }: NodeProps<PersonNodeData>) => {
  ...
  // Determinar a cor da borda baseado no tipo e status
  ...
  return (
    ...
    >
      {/* Handles - cada posição precisa de source E target */}
      {/* Handle TOP TARGET: recebe linhas de filiação dos pais (de cima para baixo) */}
      <Handle 
        type="target" 
      ...
      />
      {/* Handle TOP SOURCE: envia linhas de irmãos (estrutura em ponte) */}
      <Handle 
        type="source" 
        position={Position.Top} 
        id="top-source" 
        style={{ top: -1, left: '50%', background: 'transparent', border: 'none' }} 
      />
      {/* Handle TOP SOURCE como TARGET também: recebe linhas de irmãos (estrutura em ponte) */}
      <Handle 
        type="target" 
        position={Position.Top} 
        id="top-source" 
