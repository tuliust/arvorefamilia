...
function ConteudoTopicCard({ topico, categoria, autor }: Omit<ForumTopicCardProps, 'href'>) {
  ...
  return (
    <Card className="h-full min-w-0 transition hover:border-blue-200 hover:shadow-md">
      <CardHeader className="p-4 pb-2">
        <div className="flex flex-wrap items-center gap-2 text-xs text-gray-500">
          {categoriaResolvida?.nome && <span className="break-words">{categoriaResolvida.nome}</span>}
      ...
      <CardContent className="p-4 pt-0">
        <p className="line-clamp-2 break-words text-sm leading-6 text-gray-600">{topico.conteudo}</p>
        <div className="mt-4 flex flex-wrap items-center gap-3 text-xs text-gray-500">
          <span className="break-all">{autor ?? `Familiar ${topico.autor_id.slice(0, 8)}`}</span>
...
export function ForumTopicCard(props: ForumTopicCardProps) {
  ...
  return (
    <Link to={props.href} className="block min-w-0">
